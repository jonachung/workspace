...
/**
 * This class creates the board for the checkers game. The properties
 * of a checkerBoard object are the checkerBoard array, whether a piece moved or not
 * in a player's turn, which piece is going to be moved in a player's turn, and whose turn it is 
 * during the game.
 * 
 ...
 * In the main method, the board object is created, which in turn 
 ...
 * method until the game is over. The player with the black pieces
 ...
 * 
 * The game is over when there are no red or black pieces left on the board.
...
public class Board {
	...
	private boolean blacksTurn; // should initially be true
	 ...
	 * testing purposes. During testing purposes, the checkerBoard array would be initialized with null values. 
	 * The constructor sets the instance variables blacksTurn to true, pieceCaptured to false,
	 * pieceMoved to false, and selectedPiece, the piece selected to move, to null.
	 * However, if test is false, the checker board GUI is initialized with the checker pieces
	 * at the starting positions for checkers.
	 * 
	 ...
	 * @param currentPiece			Piece object that is being judged whether it can move or not
	 ...
	 * If the variable is initialized and the validMove method returns true, the method returns true and the square can be selected.
	 * If the variable is initialized, the move made before was a capture, the validMove method returns true, and the move being made is a capture,
	...
	public boolean canSelect(int row, int column)
	{
		if (row >= 0 && row < checkerBoard.length && column >= 0 && column < checkerBoard.length)
		{
	        ...
	        if ((spaceSelected != null) && (spaceSelected.isRed() != blacksTurn)) 
	        {
	            if (!pieceMoved) 
	            {
	                return true;
	        ...
	        } else if (spaceSelected == null)  
	        {
	            ...
	            {
	                return true; // change part of method
	            }
	        }
		}
	 ...
	 * white on the GUI. The variable selectedPiece is initialized to the piece object.
	 *  
	 ...
	 * location would be removed using the removePiece method. The pieceCaptured instance
	 * variable would be set to true if a capture was performed. When performing a capture, the 
	 * new piece location would be updated the same way as if
	...
	{	
		if (this.pieceAt(row, column) != null)
		{
			...
			StdDrawPlus.filledSquare(column + .5, checkerBoard.length - row - .5, 0.5); // put it in new method
	 ...
	 * to null. pieceCaptured wil be assigned to false.
	 */
	public void endTurn()
	{
	...
	/**
	 * Checks the piece that recently moved in game to see if the piece 
	 * should become a king or not. In order for a piece to become a king,
	 * the piece has to either be a red piece and on the top of the board
	 * OR a black piece and on the bottom of the board.
	 * 
	 * After the piece becomes a king, the image on the checkerBoard GUI will
	 * be updated with the correct image of the king piece.
	 */
	public void checkKing()
	{
		if (pieceMoved) // if piece is king then should not check if piece should be king or not.
		{
