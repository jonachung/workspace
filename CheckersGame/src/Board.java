...
public class Board {
	...
	{
		if (row > checkerBoard.length - 1 || row < 0 || col > checkerBoard.length - 1 || col < 0)
		{
	...
	{
		if (row >= 0 && row < checkerBoard.length && column >= 0 && column < checkerBoard[0].length)
		{
			Piece spaceSelected = pieceAt(row, column);
	        ...
	        {
	            if (selectedPiece != null && !pieceMoved &&
	                    validMove(row, column, selectedPiece)) 
	            {
	                return true;
	            } else if (selectedPiece != null && pieceCaptured &&
	                    validMove(row, column, selectedPiece)) 
	            {
	                return true;
	            }
	...
	{	
    	...
    	} else 
    	{
    		...
    		selectedPiece.movePiece(row, column);
        	selectedX = row;
        	selectedY = column;
        	pieceMoved = true;
    	}
